...
if [ $# = 0 ]; then
	...
	do
		...
		sed -n "/^-- Table structure for table \`$tablename\`/,/^-- Table structure for table/p" $1 > $TMP_DIR/$tablename.sql
		sed -e :a -e '$d;N;2,3ba' -e 'P;D' $TMP_DIR/$tablename.sql > $TARGET_DIR/$tablename.sql
		TABLE_COUNT=$((TABLE_COUNT+1))
		rm $TMP_DIR/$tablename.sql
	done;
elif [ $# = 2  ]; then
